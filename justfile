...
_test target max-seconds:
    ...
    {{ platform-env }} timeout --foreground --signal=TERM --kill-after=5s {{ max-seconds }}s gerbil {{ test-runtime-options }} test -v 5 "{{ target }}" 2>&1 | tee "$test_log"
...
benchmark-class-proto width='1000' samples='3':
    POO_SLOT_BENCH_WIDTH="{{ width }}" POO_SLOT_BENCH_SAMPLES="{{ samples }}" {{ platform-env }} timeout --foreground --signal=TERM --kill-after=5s 300s gxi {{ test-runtime-options }} ./t/class-proto-benchmark.ss
...
benchmark-object-scale:
    ...
    {{ platform-env }} timeout --foreground --signal=TERM --kill-after=5s 300s gxc -d "$bench_dir" -exe -o "$bench_dir/object-scale" t/object-scale-benchmark.ss
    {{ platform-env }} timeout --foreground --signal=TERM --kill-after=5s 90s "$bench_dir/object-scale"
