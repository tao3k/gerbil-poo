...
(def object-test
  (test-suite "test suite for clan/poo/object"
    ...
    (test-case "simple tests from poo.md"
      ...
      (check-equal? (.all-slots/sort foo) '(x y))
      (check-equal? (.all-slots/sort bar) '(x))
    ...
    (test-case "incremental slot storage preserves list snapshots and duplicate keys"
      (def object
        (make-object slots: (list (cons 'a ($constant-slot-spec 1))
                                 (cons 'a ($constant-slot-spec 2)))
                     defaults: (list (cons 'd #f))))
      (def first-snapshot (object-slots object))
      (.putslot! object 'a ($constant-slot-spec 3))
      (.putslot! object 'b ($constant-slot-spec 4))
      (def second-snapshot (object-slots object))
      (check-equal? (map car first-snapshot) '(a a))
      (check-equal? (map car second-snapshot) '(a a b))
      (check-equal? (map (lambda (entry) ($constant-slot-spec-value (cdr entry)))
                         second-snapshot) '(3 2 4))
      (.putslot! object 'b ($constant-slot-spec 5))
      (check-equal? (map (lambda (entry) ($constant-slot-spec-value (cdr entry)))
                         second-snapshot) '(3 2 4))
      (check-equal? (map (lambda (entry) ($constant-slot-spec-value (cdr entry)))
                         (object-slots object)) '(3 2 5))
      (def default-snapshot (object-defaults object))
      (.putdefault! object 'd #t)
      (.putdefault! object 'e #f)
      (check-equal? default-snapshot '((d . #f)))
      (check-equal? (object-defaults object) '((d . #t) (e . #f)))
